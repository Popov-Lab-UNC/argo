...
def pdist_tanimoto(smiles):
    fps, _ = get_fps(smiles, None, func="morgan")
    dists = []
...
def _filter_by_diversity(new_smiles, prev_smiles_fps, threshold=1.0):
    if len(new_smiles) == 0:
        return new_smiles
    dists = bulk_fp_tanimoto_similarity(get_fps(new_smiles, None, func="morgan")[0], prev_smiles_fps, pooling="max")
...
def bias_set_filter_and_generate(bias_smiles,
    ...
    while num_hits < tot_hits:
        ...
        if diverse_thresh is not None:
            batch_hits = _filter_by_diversity(batch_hits, denovo_fps, threshold=diverse_thresh)
        denovo_hits.extend(batch_hits)
        denovo_fps.extend(get_fps(batch_hits, None, func="morgan")[0])
        num_hits += len(batch_hits)
...
class GEMModel:
    """
    Class-based interface for GEM generative modeling, wrapping filter_and_generate and bias_set_filter_and_generate.
    """
    def __init__(self,
                 gen_model_path: str,
                 clf_model_path: Optional[str] = None,
    ...
    def generate(self,
        ...
        smiles: list of SMILES strings (and labels if mode='filter')
        labels: list of labels (required for mode='filter')
        Other arguments are passed to the underlying function.
        """
        if mode == 'filter':
            if smiles is None or labels is None:
                raise ValueError("For mode='filter', both smiles and labels must be provided.")
            return filter_and_generate(
                ...
                cluster_biasing_set=cluster_biasing_set,
                cluster_threshold=cluster_threshold,
                fine_tune_epochs=fine_tune_epochs,
                fine_tune_lr=fine_tune_lr,
                conf_thresh=conf_thresh,
                diverse_thresh=diverse_thresh,
                gen_model_tuned=gen_model_tuned,
                save_models=self.save_models,
                save_auxiliary_files=save_auxiliary_files,
            ...
            if smiles is None:
                raise ValueError("For mode='bias_set', smiles must be provided.")
            return bias_set_filter_and_generate(
                bias_smiles=smiles,
                gen_model_path=self.gen_model_path,
                clf_model_path=self.clf_model_path,
                do_filter=do_filter,
            ...
            raise ValueError(f"Unknown GEM generation mode: {mode}")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--model_path", type=str, required=False, default=f'{os.path.dirname(__file__)}/pretrained/pretrained_chembl.pt',
